...
import { parseResumeFile, generateResume } from '../lib/gemini'
        ...
        <main className="max-w-5xl mx-auto px-4 py-8">
          <div className="grid lg:grid-cols-[300px_1fr] gap-6">
            ...
            <aside className="lg:sticky lg:top-20 h-fit space-y-4">
              ...
              {result.total_years_experience != null && result.total_years_experience >= 5 && (
                <p className="text-xs" style={{ color: '#6B8CAE' }}>With {result.total_years_experience}+ years, this may run to a second page. That's fine for senior profiles.</p>
              )}
            </aside>

            {/* The resume */}
            <div className="overflow-x-auto rounded-2xl" style={{ background: '#525659', padding: 16 }}>
      ...
      <main className="max-w-2xl mx-auto px-5 py-10">
        ...
        <div className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-3">
            <Field label="Full name" value={form.name} onChange={e => update({ name: e.target.value })} placeholder="Aman Jindal" />
            <Field label="Headline" value={form.title} onChange={e => update({ title: e.target.value })} placeholder="Product Operations Manager | CSPO" />
            <Field label="Location" value={form.location} onChange={e => update({ location: e.target.value })} placeholder="Delhi NCR" />
            <Field label="Email" value={form.email} onChange={e => update({ email: e.target.value })} placeholder="you@email.com" />
            <Field label="Phone" value={form.phone} onChange={e => update({ phone: e.target.value })} placeholder="+91-..." />
            <Field label="LinkedIn" value={form.linkedin} onChange={e => update({ linkedin: e.target.value })} placeholder="linkedin.com/in/…" />
            <Field label="GitHub / Portfolio" value={form.github} onChange={e => update({ github: e.target.value })} placeholder="github.com/… or a portfolio URL" />
              ...
              {form.experience.map((e, i) => (
                <div key={i} className="rounded-2xl p-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                  <div className="grid sm:grid-cols-2 gap-2 mb-2">
                    <Field label="Role" value={e.role} onChange={ev => updateExp(i, { role: ev.target.value })} placeholder="Digital Product Manager" />
                    <Field label="Company" value={e.company} onChange={ev => updateExp(i, { company: ev.target.value })} placeholder="Deloitte" />
                    <Field label="Start" value={e.start} onChange={ev => updateExp(i, { start: ev.target.value })} placeholder="Jun 2025" />
                    <Field label="End" value={e.end} onChange={ev => updateExp(i, { end: ev.target.value })} placeholder="Present" />
                  </div>
                  <Area label="What you did" hint="one point per line" value={e.bullets} onChange={ev => updateExp(i, { bullets: ev.target.value })} placeholder={'Owned end-to-end delivery for…\nDrove adoption up by…'} />
          ...
          <Area label="Skills" hint="Category: skill, skill — one per line" value={form.skills} onChange={e => update({ skills: e.target.value })} placeholder={'Product: Roadmapping, Agile, GTM\nTools: Power BI, JIRA, Salesforce'} />
          <Area label="Education" hint="Degree - Institution | Years/CGPA — one per line" value={form.education} onChange={e => update({ education: e.target.value })} placeholder={'PGDM - LBSIM | 2021–2023 | 8.5/10'} />
          <Area label="Certifications" hint="one per line" value={form.certifications} onChange={e => update({ certifications: e.target.value })} placeholder={'CSPO - Scrum Alliance, 2026'} />
          <Area label="Achievements" hint="one per line" value={form.achievements} onChange={e => update({ achievements: e.target.value })} placeholder={'Gold Medalist, PGDM 2023'} />
